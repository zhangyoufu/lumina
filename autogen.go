...
func readUint32(r Reader) (v uint32, err error) {
    ...
    if b0, err = r.ReadByte(); err != nil { return }
    if b0 & 0x80 == 0 {
        // 0x00000000 ~ 0x0000007F
        v = uint32(b0)
        return
    }
    if b1, err = r.ReadByte(); err != nil { return }
    if b0 & 0x40 == 0 {
        // 0x00000080 ~ 0x00003FFF
        v = uint32(b0 & 0x3F) << 8 | uint32(b1)
        if v < 0x80 { err = errUnexpectedEncoding }
        return
    }
    if b2, err = r.ReadByte(); err != nil { return }
    if b3, err = r.ReadByte(); err != nil { return }
    if b0 & 0x20 == 0 {
        // 0x00004000 ~ 0x1FFFFFFF
        v = uint32(b0 & 0x1F) << 24 | uint32(b1) << 16 | uint32(b2) << 8 | uint32(b3)
        if v < 0x4000 { err = errUnexpectedEncoding }
        return
    }
    if b4, err = r.ReadByte(); err != nil { return }
    // 0x20000000 ~ 0xFFFFFFFF
    v = uint32(b1) << 24 | uint32(b2) << 16 | uint32(b3) << 8 | uint32(b4)
    if v < 0x20000000 /* || b0 & 0x1F != 0 */ { err = errUnexpectedEncoding }
    return
}
...
func writeUint32(w Writer, v uint32) (err error) {
    ...
    case v <= 0x0000007F:
    case v <= 0x00003FFF:
        if err = w.WriteByte(0x80 | byte(v >> 8)); err != nil { return }
    case v <= 0x1FFFFFFF:
        if err = w.WriteByte(0xC0 | byte(v >> 24)); err != nil { return }
        if err = w.WriteByte(byte(v >> 16)); err != nil { return }
        if err = w.WriteByte(byte(v >> 8)); err != nil { return }
    default:
        if err = w.WriteByte(0xE0); err != nil { return }
        if err = w.WriteByte(byte(v >> 24)); err != nil { return }
        if err = w.WriteByte(byte(v >> 16)); err != nil { return }
        if err = w.WriteByte(byte(v >> 8)); err != nil { return }
    }
...
func writeUint64(w Writer, v uint64) (err error) {
    if err = writeUint32(w, uint32(v)); err != nil { return }
    if err = writeUint32(w, uint32(v >> 32)); err != nil { return }
    return
}
...
func readBool(r Reader) (bool, error) {
    ...
    if t > 1 { return false, errUnexpectedEncoding }
    return t != 0, nil
}
...
func writeString(w Writer, s string) error {
    if pos := strings.IndexByte(s, 0); pos != -1 { return errInvalidValue }
    if _, err := strings.NewReader(s).WriteTo(w); err != nil { return err }
