...
func (this *PullMdPacket) ReadFrom(r Reader) (err error) {
    ...
    // Slice []PatternId
    var v4 uint32
    if v4, err = readUint32(r); err != nil { return }
    this.PatternIds = make([]PatternId, v4)
    for i := uint32(0); i < v4; i++ {
        // Field this.PatternIds[i]
        // Struct PatternId
        if err = this.PatternIds[i].ReadFrom(r); err != nil { return }
    }
    return
}
...
func (this *PullMdPacket) WriteTo(w Writer) (err error) {
    ...
    // Field this.Keys
    // Slice []MdKey
    if len(this.Keys) > 0x7FFFFFFF { err = errTooLong; return }
    ...
    for i := uint32(0); i < v2; i++ {
        ...
        // Basic uint32
        // Typed MdKey
        if err = writeUint32(w, uint32(this.Keys[i])); err != nil { return }
    }
    // Field this.PatternIds
    // Slice []PatternId
    if len(this.PatternIds) > 0x7FFFFFFF { err = errTooLong; return }
    var v4 = uint32(len(this.PatternIds))
    if err = writeUint32(w, v4); err != nil { return }
    for i := uint32(0); i < v4; i++ {
        // Field this.PatternIds[i]
        // Struct PatternId
        if err = this.PatternIds[i].WriteTo(w); err != nil { return }
    }
    return
}
